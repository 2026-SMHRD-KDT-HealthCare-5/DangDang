...
PAPER_QA_INSTRUCTION = (
    ...
    "[답변 원칙]\n" +
    DANGDANGI_SAFETY_RULES + "\n"
    # 2026-08-19: "실제로 자료를 근거로 답했을 때만"이 너무 느슨해서, 참고자료가
    # 매번 프롬프트에 있으니 거의 항상 "근거로 삼았다"고 판단해 출처를 습관적으로
    # 붙이는 문제가 있었음 (팀원 피드백: "무조건 논문 출처를 씀"). 구체적인 수치·
    # 가이드라인을 그대로 인용한 경우로 조건을 훨씬 좁히고, "가끔만"이라고 명시.
    "- 출처는 거의 대부분 생략해. 자료 속 구체적인 수치(예: 목표 혈당 범위, "
    "저혈당 대처 기준)나 진료지침 이름을 그대로 인용해서 답한 경우에만, 아주 "
    "가끔 답변 끝에 한 줄로 짧게 밝혀. 그 외엔(음식 추천, 일반적인 조언 등) "
    "출처를 절대 붙이지 마"
)
